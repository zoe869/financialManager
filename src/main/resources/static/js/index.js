...
function getAllPayways() {
    ...
    $.get("/bills/getPayways",function (res) {
        var html = '<option value="-1">choose payment method</option>';
        if (res.code == 200){
...
//获取income的recordTitle
function getIncomeTypes() {
    $.ajaxSettings.async = false;
    $.get("/bills/getPaymentTypes",function (res) {
        var html = '<option value="-1">choose payment content</option>';
        if (res.code == 200){
...
function getAllWays() {
    ...
    $.get("/bills/getAllWays",function (res) {
        var html = '<option value="-1">choose risk level</option>';
        if (res.code == 200){
            $.each(res.datas,function (index,item) {
